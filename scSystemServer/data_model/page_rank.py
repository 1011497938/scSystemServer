...
def loadPageRank(person_array):
    page_rank = open('scSystemServer/data_model/data/pageRank.json', 'r', encoding='utf-8').read()
    page_rank = json.loads(page_rank)
    for person in person_array:
        person.page_rank = page_rank[person.id]
    return #page_rank

def savePageRank(event_array, person_array):
    ...
    open('scSystemServer/data_model/data/pageRank.json', 'w', encoding='utf-8').write(json.dumps(person_rank, indent=2, ensure_ascii = False))
    return person_rank
