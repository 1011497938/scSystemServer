...
class Dataset(object):
    def __init__(self,random_walks,node_type_mapping,window_size):
        index2token,token2index,word_and_counts,index2frequency,node_context_pairs= self.parse_random_walk(random_walks,window_size)
        self.window_size = window_size
        self.nodeid2index = token2index
        self.index2nodeid = index2token
        self.index2frequency = index2frequency
        index2type,type2indices = self.parse_node_type_mapping(node_type_mapping, self.nodeid2index)
    ...
    def parse_random_walk(self,random_walks,window_size):
        ...
        for sent in random_walks:
            sent = [token2index[word] for word in sent if word in token2index]
    ...
    def parse_node_type_mapping(self,node_type_mapping,nodeid2index):
        #this method does not modify any class variables
        index2type={}
        
        for pair in node_type_mapping:
            if len(pair[0])==0 or len(pair[1])==0:
                print("something is wrong!!!")
                print(pair)
                continue
            # print(nodeid2index[pair[0]])
            # print(index2type[nodeid2index[pair[0]]])
    ...
    def get_one_batch(self):
        if self.count %10000==0:
            print(self.count, '/', len(self.node_context_pairs), '=', self.count/len(self.node_context_pairs))
...
if __name__ == '__main__':
    #test code  
    dataset=Dataset(random_walks="../data/test_data/random_walks.txt",node_type_mapping="../data/test_data/node_type_mapings.txt",window_size=1)
